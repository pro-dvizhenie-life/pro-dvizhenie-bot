...
export const DynamicStep: FC = () => {
    ...
    const setVal = (v: any) => dispatch(setAnswer({ code: currentQ.code, value: v }))
    const next = () => { void dispatch(advance()) }
    const answerAndNext = (v: any) => { setVal(v); void dispatch(advance()) }
        ...
        <Stack gap="md">
            ...
            {currentQ.code === 'q_who_fills' && (
                <Stack gap="sm">
                    ...
                    {fieldError && <Text c="red" size="sm">{fieldError}</Text>}
            ...
            {isYesNo(currentQ) && currentQ.code !== 'q_who_fills' && (
                <Group gap="sm">
                    ...
                    <ButtonUI onClick={() => answerAndNext(false)} color="gray">Нет</ButtonUI>
                </Group>
            )}
            ...
            {isSelect(currentQ) && currentQ.code !== 'q_who_fills' && (
                <Stack gap="xs">
                    <Group gap="sm">
                        {getOptions(currentQ).map((opt) => (
                            <ButtonUI key={String(opt.value)} onClick={() => answerAndNext(opt.value)}>
                                {opt.label}
                            </ButtonUI>
                        ))}
                    </Group>
                ...
                <Stack gap="xs">
                    <Group gap="sm">
                        {NEED_CONSULTING_OPTIONS.map((opt) => (
                            <ButtonUI key={opt.value} onClick={() => answerAndNext(opt.value)}>
                                {opt.label}
                            </ButtonUI>
                        ))}
                    </Group>
